...
@section('title')
    @if(isset($taskdata))
        {{$deal->loan_ref}} :: Edit Deal Task
    @else
        {{$deal->loan_ref}} :: Add Deal Task
    @endif
@endsection
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="main-card mb-3 card">
                <div class="card-body">
                    <form method="post"
                          action="{{isset($taskdata)?route('admin.dealtsk.update', [encrypt
                          ($taskdata->id),
                          encrypt($deal->id)]):route('admin.dealtsk.post',encrypt($deal->id))}}"
                                    ...
                                    <select name="followup_detail" id="followup_detail" class="multiselect-dropdown form-control">
                                        ...
                                        @if(count($dealDetails)>0)
                                            @foreach($dealDetails as $ddkey => $dealDetail)
                                                <option value="{{$ddkey}}"
                                                    {{isset($taskdata) &&  $taskdata->followup_detail ==
                                                    $ddkey ? 'selected="selected"' : ''}}>
                                                    {{$dealDetail}}
                                                </option>
                                            @endforeach
                                        @endif
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(!$.isEmptyObject(data.error)){
                        ...
                        setTimeout(function(){
                            window.location="{{route('admin.dealtsk.list', encrypt($deal->id))}}"
                        },3000);
                        hideLoader();
