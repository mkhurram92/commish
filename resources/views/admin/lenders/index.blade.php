            ...
            <form class="form-inline" id="addForm" method="post" action="">
                ...
                <div class="col-sm-4">
                    <div class="mb-2 mr-sm-2 mb-sm-0 position-relative form-group">
                        ...
                        <input name="code" id="code" placeholder="" type="text" class="form-control">
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="mb-2 mr-sm-2 mb-sm-0 position-relative form-group">
                        <label for="commis_type" class="mr-sm-2">Commis. type</label>
                        <select name="commis_type" id="commis_type" placeholder="" class="form-control">
                            <option value="1">%</option>
                            <option value="2">Fee per deal</option>
                        </select>
                    </div>
                </div>
        ...
        <div class="col-sm-12">
            <div class="">
                <table style="width: 100%" id="TableData" class="table table-hover table-striped table-bordered"
                    ...
                    data-sAjaxSource="{{ route('admin.lenders.getrecords') }}"
                    data-aoColumns='{"mData": "Index no"},{"mData": "Name"},{"mData": "Code"},
                    {"mData": "Added On"},{"mData": "Modified On"},{"mData": "Action"}'>
                    <thead>
                        <tr>
                            <th class="no-sort">#</th>
                            <th class="noVis">Name</th>
                            <th class="noVis">Code</th>
    ...
    $(document).ready(() => {
            $("#addForm").validate({
                ...
                submitHandler: function(){
                    ...
                    $.ajax({
                        url: "{{ route('admin.lenders.post') }}",
                        type:'POST',
                        data:  {
                        "_token": "{{ csrf_token() }}",
                            "name": jQuery('#name').val(),
                            "code": jQuery('#code').val(),
                            "commis_type": jQuery('#commis_type').val()
                             },
                        success: function(data) {
                            ...
                            if($.isEmptyObject(data.error)){
                                ...
                                jQuery('#commis_type').val(1).removeClass('is-invalid').removeClass('is-valid');
                                hideLoader();
                                refreshTable({});
                            }else{
        ...
        function editRecord(current)
        {
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                showLoader();
                $.ajax({
                    url: "{{ route('admin.lenders.getrecord') }}",
                    type:'POST',
                    data:  {
                        "_token": "{{ csrf_token() }}",
                        "id": id
                    ...
                    success: function(data) {
                        ...
                        if($.isEmptyObject(data.error)){
                            ...
                            if(typeof payloadData != "undefined" && Object.keys(payloadData).length>0)
                            {
                                ...
                                jQuery('#edit_code').val(payloadData.code);
                                jQuery('#edit_commis_type').val(payloadData.commis_type);
                                hideLoader();
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.lenders.delete') }}",
                            type:'POST',
                            data:  {
                                "_token": "{{ csrf_token() }}",
                                "id": id
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Lender Details : Edit</h5>
            ...
            <form method="post" action="" id="editForm">
                <div class="modal-body">
                    ...
                    <div class="col-sm-12">
                        <div class="mb-2 mr-sm-2 mb-sm-0 position-relative form-group">
                            <label for="code" class="mr-sm-2">Code</label>
                            <input name="code" id="edit_code" placeholder="" type="text" class="form-control">
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="mb-2 mr-sm-2 mb-sm-0 position-relative form-group">
                            <label for="edit_commis_type" class="mr-sm-2">Commis. type</label>
                            <select name="commis_type" id="edit_commis_type" placeholder="" class="form-control">
                                <option value="1">%</option>
                                <option value="2">Fee per deal</option>
                            </select>
                        </div>
    ...
    jQuery(document).ready(function(){
            $("#editForm").validate({
                ...
                submitHandler: function(){
                    ...
                    showLoader();
                    $.ajax({
                        url: "{{ route('admin.lenders.update') }}",
                        type:'POST',
                        data:  {
                            "_token": "{{ csrf_token() }}",
                            "name": jQuery('#edit_name').val(),
                            "code": jQuery('#edit_code').val(),
                            "id" :jQuery('#edit_id').val(),
                            "commis_type" :jQuery('#edit_commis_type').val(),
                        },
                        success: function(data) {
                            ...
                            if($.isEmptyObject(data.error)){
                                ...
                                jQuery('#edit_id').val('');
                                jQuery('#edit_code').val('').removeClass('is-invalid').removeClass('is-valid');
                                jQuery('#edit_code').val('');
                                jQuery('#edit_commis_type').val(1);
