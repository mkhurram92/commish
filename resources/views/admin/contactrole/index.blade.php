    ...
    <div class="page-header">
        ...
        <div class="page-rightheader ml-auto d-lg-flex d-none">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">                             Contact Role
                </li>
            </ol>
        </div>
    </div>
           ...
           <div class="col-sm-12">
               <div class="">
                   <table style="width: 100%"  id="TableData" class="table table-hover table-striped table-bordered"
                           data-toggle="table" data-height="500" data-show-columns="true" data-sAjaxSource="{{ route('admin.contactrole.getrecords') }}"
    ...
    <script>
        $(document).ready(() => {
            $("#addForm").validate({
                ...
                submitHandler: function(){
                    ...
                    $.ajax({
                        url: "{{ route('admin.contactrole.post') }}",
                        type:'POST',
                        data:  {
                        "_token": "{{ csrf_token() }}",
        ...
        function editRecord(current)
        {
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                showLoader();
                $.ajax({
                    url: "{{ route('admin.contactrole.getrecord') }}",
                    type:'POST',
                    data:  {
                        "_token": "{{ csrf_token() }}",
                        "id": id
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.contactrole.delete') }}",
                            type:'POST',
                            data:  {
                                "_token": "{{ csrf_token() }}",
                                "id": id
        ...
        jQuery(document).ready(function(){
            $("#editForm").validate({
                ...
                submitHandler: function(){
                    ...
                    showLoader();
                    $.ajax({
                        url: "{{ route('admin.contactrole.update') }}",
                        type:'POST',
                        data:  {
                            "_token": "{{ csrf_token() }}",
