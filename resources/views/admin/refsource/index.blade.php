           ...
           <div class="col-sm-12">
               <div class="">
                   <table style="width: 100%"  id="TableData" class="table table-hover table-striped table-bordered"
                           data-toggle="table" data-height="500" data-show-columns="true" data-sAjaxSource="{{ route('admin.referral.getrecords') }}"
    ...
    <script>
        ...
        function editRecord(current)
        {
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                showLoader();
                $.ajax({
                    url: "{{ route('admin.referral.getrecord') }}",
                    type:'POST',
                    data:  {
                        "_token": "{{ csrf_token() }}",
                        "id": id
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                ...
                swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.referral.delete') }}",
                            type:'POST',
                            data:  {
                                "_token": "{{ csrf_token() }}",
                                "id": id
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">Referral Source :: Edit</h5>
        ...
        jQuery(document).ready(function(){
            $("#editForm").validate({
                ...
                submitHandler: function(){
                    ...
                    showLoader();
                    $.ajax({
                        url: "{{ route('admin.referral.update') }}",
                        type:'POST',
                        data:  {
                            "_token": "{{ csrf_token() }}",
