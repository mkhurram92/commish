...
<table style="margin-top: 5px;margin-bottom:5px;width: 100%">
    <tbody>
    <tr>
        <td style="width: 30%"><span style="font-size: 18px;font-weight: bold;">FM Direct Trail Outstanding</span></td>
        <td><span style="width: 60%;">Grouped By {{ $group_by }} For period from {{ $date_from.' to '.$date_to }}</span>
        </td>
    </tr>
    </tbody>
</table>
...
$total_ABP_est_trail_amt = 0;
$total_ABP_actual_trail_amt = 0;
$total_ABP_trail_diff = 0;
    ...
    @foreach($deals as $deal)
        ...
        $deals_list = \App\Models\Deal::select('deals.*')->with(['lender', 'client', 'deal_status', 'product', 'broker_staff'])
            ...
            ->where(function ($query) {
                $query->where('broker_est_trail', '>', 0)->orWhere('broker_est_loan_amt', '>', 0);
            });
        ...
        <table style="width: 100%;margin-top: 5px">
            ...
            <tr>
                ...
                <th style="width: 7%">ABP Est. Trail</th>
                <th style="width: 7%">Actual <span style="margin-right: 10px">Trail</span></th>
                <th style="width: 7%">Trail Difference</th>
            ...
            @foreach($deals_list as $k=>$deal_list)
                ...
                @if($trail)
                    ...
                    $ABP_est_trail_amt += $deal_list->broker_est_trail;
                    $ABP_actual_trail_amt += $trail;
                    $ABP_trail_diff += $t_tra_dif;
            ...
            <tr>
                ...
                <th style="text-align: left;width: 7%">${{$ABP_est_trail_amt}}</th>
                <th style="text-align: left;width: 7%">${{ $ABP_actual_trail_amt }}</th>
                <th style="text-align: left;width: 7%">${{ $ABP_trail_diff }}</th>
            ...
            $total_ABP_est_trail_amt += $ABP_est_trail_amt;
            $total_ABP_actual_trail_amt += $ABP_actual_trail_amt;
            $total_ABP_trail_diff += $ABP_trail_diff;
...
@else
    <table style="width: 100%;margin-top: 5px">
        ...
        <tr>
            ...
            <th style="width: 4%">Date Settled</th>
            <th style="width: 7%">ABP Est. trail</th>
            <th style="width: 7%">Actual trail</th>
            <th style="width: 7%">trail Difference</th>
...
@if(count($deals)>0)
    ...
    <table class="grand_total" style="width: 100%">
        <tr>
            ...
            <th style="text-align: left;width: 7%"></th>
            <th style="text-align: left;width: 7%">${{$total_ABP_est_trail_amt}}</th>
            <th style="text-align: left;width: 7%">${{ $total_ABP_actual_trail_amt }}</th>
            <th style="text-align: left;width: 7%">${{ $total_ABP_trail_diff }}</th>
