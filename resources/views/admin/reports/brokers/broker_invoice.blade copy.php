...
<body style=" font-family: system-ui, system-ui, sans-serif;">
    ...
    @if (count($brokers) > 0)
        @foreach ($brokers as $broker)
            ...
            <br>
            <tr>
                <td style="font-size: 12px;background-color: #f8f8f8;">
                    <b>For Commission due for the period : {{ date('d/m/Y', strtotime($date_from)) }} to
                        {{ date('d/m/Y', strtotime($date_to)) }}</b>
                </td>
            </tr>
            <br>
            <?php
                ...
                <tbody class="body_class">
                    ...
                    @foreach ($broker_staffs as $index => $broker_staff_obj)
                        ...
                        @if ($nameeee != $older_broker_name)
                            <?php
                                //$deal_id = $broker_staff_obj->deal->id;
                                $total_actual_loan += $broker_staff_obj->deal->actual_loan;
                                $total_agg_amount += $broker_staff_obj->deal->agg_amount;
                            ...
                            @if ($older_broker_name != 0)
                                <tr style="background-color: #f2f2f2;">
                                    ...
                                    <td><b>${{ number_format((float) $total_actual_loan, 2) }}</b>
                                    </td>
                                    <td colspan="3"></td>
                                    <td>
                                        <b>${{ number_format((float) $total_total_amount, 2) }}</b>
                                    </td>
                                    <td>
                                        <b>${{ number_format((float) $total_agg_amount, 2) }}</b>
                                    </td>
                                    <td>
                                        <b>${{ number_format((float) $total_referror_amount, 2) }}</b>
                                    </td>
                                    <td>
                                        <b>${{ number_format((float) $total_broker_amount, 2) }}</b>
                                    </td>
                                </tr>
                            @endif
                            <?php
                            ...
                            <tr style="border-bottom: 1px solid; padding-bottom: 5px;">
                                ...
                                <td style="border-bottom: 1px solid; padding-bottom: 5px;" colspan="8"><span
                                        style="background-color: #ffff99;">{{ $nameeee }}</span></td>
                            </tr>
                        @endif
                        <tr>
                            <td>
                    ...
                    <tr style="background-color: #f2f2f2;">
                        ...
                        <td>
                            <b>${{ number_format((float) $total_actual_loan, 2) }}</b>
                        </td>
                        <td colspan="3"></td>
                        <td>
                            <b>${{ number_format((float) $total_total_amount, 2) }}</b>
                    ...
                    <tr style="background-color: #f2f2f2;">
                        ...
                        <td><b>${{ number_format($total_actual_loan, 2) }}</b></td>
                        <td colspan="3"></td>
                        <td><b>${{ number_format($total_total_amount, 2) }}</b></td>
                        <td><b>${{ number_format($total_agg_amount, 2) }}</b></td>
                        <td><b>${{ number_format($total_referror_amount, 2) }}</b></td>
                        <td><b>${{ number_format($total_broker_amount, 2) }}</b></td>
                    </tr>
                </tbody>
            </table>
