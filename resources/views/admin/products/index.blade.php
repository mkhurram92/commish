           ...
           <div class="col-sm-12">
               <div class="">
                   <table style="width: 100%"  id="TableData" class="table table-hover table-striped table-bordered"
                           data-toggle="table" data-height="500" data-show-columns="true" data-sAjaxSource="{{ route('admin.products.getrecords') }}"
    ...
    <script>
        ...
        function editRecord(current)
        {
            ...
            if(typeof id != "undefined" && parseInt(id) > 0)
            {
                showLoader();
                $.ajax({
                    url: "{{ route('admin.products.getrecord') }}",
                    type:'POST',
                    data:  {
                        "_token": "{{ csrf_token() }}",
                        "id": id
