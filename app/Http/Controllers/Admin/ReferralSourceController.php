...
class ReferralSourceController extends Controller
{
    public function index() {
        ...
        return view('admin.refsource.index');
    }

    public function saveData(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => "required|unique:referral_sources"
        ]);
        ...
        $refSource = new ReferralSource();
        $refSource->name = $request['name'];
        $refSource->created_at = Carbon::now('utc')->toDateTimeString();
        $refSource->updated_at = Carbon::now('utc')->toDateTimeString();
        $refSource->save();
    ...
    public function getRecords(Request $request)
    {
        try{
            ...
            // Total records
            $totalRecords = ReferralSource::select('count(*) as allcount')->whereNull('deleted_at')->count();
             $filterSql = ReferralSource::select('count(*) as allcount')->whereNull('deleted_at');
            if($searchValue!='')
                $sql =$filterSql->where('name', 'like', '%' .$searchValue . '%');

            $totalRecordswithFilter = $filterSql->count();
            // Fetch records
            $sql = ReferralSource::select(DB::raw('id,name, DATE_FORMAT(created_at,"%Y-%m-%d %H:%i:%s") as formated_created_at, DATE_FORMAT(updated_at,"%Y-%m-%d %H:%i:%s") as formated_updated_at'))->orderBy($columnName,
    ...
    public function getRecord(Request $request)
    {
        try{
            ...
            $record = ReferralSource::find($request->id);
            if(!$record)
            {
    ...
    public function updateData(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id' => "required|integer",
            'name' => "required|unique:referral_sources,id,".$request->id
        ]);
        ...
        $refSource = ReferralSource::find($request->id);
        if(!$refSource)
        {
