...
class ProcessorsController extends Controller
{
    public function index() {
        ...
        return view('admin.processor.index');
    }
    ...
    public function getRecords(Request $request)
    {
        try{
            ...
            // Total records
            $totalRecords = Processor::select('count(*) as allcount')->whereNull('deleted_at')->count();
             $filterSql = Processor::select('count(*) as allcount')->whereNull('deleted_at');
            if($searchValue!='')
                $sql =$filterSql->where('name', 'like', '%' .$searchValue . '%');

            $totalRecordswithFilter = $filterSql->count();
            // Fetch records
            $sql = Processor::select(DB::raw('id,name, DATE_FORMAT(created_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_created_at, DATE_FORMAT(updated_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_updated_at'))->orderBy($columnName,
    ...
    public function getRecord(Request $request)
    {
        try{
            ...
            $record = Processor::find($request->id);
            if(!$record)
            {
    ...
    public function updateData(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id' => "required|integer",
            'name' => "required|unique:processors,id,".$request->id
        ]);
        ...
        $refSource = Processor::find($request->id);
        if(!$refSource)
        {
